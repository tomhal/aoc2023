pub const DATA: &str = "Game 1: 1 blue; 4 green, 5 blue; 11 red, 3 blue, 11 green; 1 red, 10 green, 4 blue; 17 red, 12 green, 7 blue; 3 blue, 19 green, 15 red
...
impl Draw {
    ...
    pub fn from_str(s: &str) -> Self {
        ...
        for part in parts {
            ...
            let num: u32 = parts2[0].parse().unwrap();

            match parts2[1] {
                "red" => result.red += num,
                "green" => result.green += num,
                "blue" => result.blue += num,
                _ => panic!("Unknown color '{}'", parts2[1]),
            }
        }
...
impl Game {
    pub fn from_str(s: &str) -> Self {
        ...
        // Read out the id
        let id_str = &parts[0][5..];
        let id: u32 = id_str.parse().unwrap();

        Game {
            id,
            draws: Game::read_draws(parts[1]),
        }
    }

    // Input: s = "3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green"
    fn read_draws(s: &str) -> Vec<Draw> {
        let mut result = Vec::new();

        let parts: Vec<&str> = s.split("; ").collect();
        for part in parts {
            let draw = Draw::from_str(part);
            result.push(draw);
        }
    ...
    pub fn minumum_limits(&self) -> Limits {
        ...
        for draw in self.draws.iter() {
            result.red = result.red.max(draw.red);
            result.green = result.green.max(draw.green);
            result.blue = result.blue.max(draw.blue);
        }
...
pub fn read_input(input: &str) -> Vec<Game> {
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
        ...
        assert_eq!(1, games[0].id, "id is not 1");
        assert_eq!(3, games[0].draws.len(), "draws is not 3");

        assert_eq!(3, games[0].draws[0].blue);
        assert_eq!(4, games[0].draws[0].red);

        assert_eq!(1, games[0].draws[1].red);
        assert_eq!(2, games[0].draws[1].green);
        assert_eq!(6, games[0].draws[1].blue);

        assert_eq!(0, games[0].draws[2].red);
        assert_eq!(2, games[0].draws[2].green);
        assert_eq!(0, games[0].draws[2].blue);
    }
}
